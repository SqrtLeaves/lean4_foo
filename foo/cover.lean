...
def is_cover {Yc Xc: Type} (Y: TopoSpace Yc) (X: TopoSpace Xc) (map :TopoMap Y X) : Prop :=
  ...
  let h_subset: U ⊆ Y.base_set := Y.opens_are_subsets_of_base U hU.2
  (is_homeo (TopoMap_restriction map (SubspaceTopology Y U h_subset) (h_subset)))

def is_connected_cover (map: TopoMap Y X) : Prop :=
  is_cover Y X map ∧ is_connected Y
